...
// Separable 2D sinc interpolation using lookup table
inline std::vector<double> sinc_interp_2d_separable(
    ...
    std::vector<double> temp_row(height, 0.0);
    ...
    for (size_t i = 0; i < n_points; ++i) {
        ...
        // First pass: interpolate in X for each row
        for (int j = 0; j < static_cast<int>(height); ++j) {
            ...
            for (int k = x0 - window_size; k <= x0 + window_size; ++k) {
                ...
                sum += signal[j][k] * sinc_x * wx;
            }
            temp_row[j] = sum;
        }

        // Second pass: interpolate in Y using the temporary row result
        double val = 0.0;
