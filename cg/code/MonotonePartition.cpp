...
enum class VERTEX_CATEGORY { START, END, REGULAR, SPLIT, MERGE, INVALID };
...
struct Edge2dDCELWrapper {
    ...
    const float computeX(const Point2d& point) const {
        float _deno = (dest[Y] - origin[Y]);
        float _x = point[X];
        if (_deno != 0) {
            _x = (point[Y] - origin[Y]) * (dest[X] - origin[X]) / _deno + origin[X];
        }
        return _x;
    }
...
static void handle_regular_vertices(Vertex2dDCELWrapper& vertex,
    ...
    Polygon2dDCEL* poly)
{
    // check interior of the polygon lies right to vertex point
...
void get_monotone_polygons(Polygon2dDCEL* poly, vector<Polygon2dDCEL*>& mono_polies)
{
    vector<Vertex2dDCELWrapper> vertices;
    ...
    sweep_point->assign(Y, vertices[0].vert->point[Y] + 1.0f);
    ...
    for (auto vertex : vertices) {
        ...
        sweep_point->assign(Y, vertex.vert->point[Y] + 1.0f);
