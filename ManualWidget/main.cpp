...
int main(int argc, char *argv[]) {
    ...
    ModelMdReader* reader = new ModelMdReader;
    string ipt = "/Users/justinchang/workspace/ShareTest/ManualWidget/doc/README.md";
    reader->SetFname(ipt);
    reader->testRun();

    MdNode* m_sRoot = reader->GetRoot();
    int maxLevel = reader->GetMaxLevel();
    ViewManual* wid = new ViewManual;
    wid->resize(640, 480);
    QTreeWidget* tw = wid->GetTreeWidget();
    ...
    reader->TraverseMdNode(m_sRoot, [&tw, &nodes, &btns, &tes](MdNode *node)
    {
        cout << *node << endl;
